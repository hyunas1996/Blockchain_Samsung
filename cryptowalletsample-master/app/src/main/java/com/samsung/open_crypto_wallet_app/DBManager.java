...
public class DBManager {
    ...
    private static void populateAccountModel(int accountId) {
        ...
        if(!SharedPreferenceManager.getUseOtherKeyManager(mNavActivityInstance)) {
            ...
            //keystore 로부터 HDPath를 가져오는 부부을 해보자.
            //ScwService.getInstance(). 를 통해서 다양한 메소드를 사용할 수 있다. HDpath는 getInstance 없이도 접근할 수 있긴 함.

            //인자 : 코인 타입, address index
            String hdPath = ScwService.getHdPath(ScwCoinType.ETH, 0);

            //logcat 에 이더리움 hdpath 가져오는 것을 볼 수 있음
            //돋보기 표시에 tag 입력해서 쉽게 확인 할 수 있다.
            Log.d("DBManager", "hdpath : " + hdPath);

            //가져온 hdPath 를 내 계정에다가 적용하는 것.
